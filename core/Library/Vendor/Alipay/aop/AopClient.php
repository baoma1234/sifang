...
class AopClient {
	...
	*/
	...
	protected function buildRequestForm($para_temp) {
        ...
        $postData = http_build_query($para_temp);
        $curl = curl_init();
        curl_setopt($curl, CURLOPT_URL,  $this->gatewayUrl."?charset=".trim($this->postCharset));
        curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, false); // stop verifying certificate
        curl_setopt($curl, CURLOPT_NOBODY, 1); //不返回 BODY 页面内容
