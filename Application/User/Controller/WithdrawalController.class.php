...
class WithdrawalController extends UserController
{
   ...
   /*  申请USDT结算
    ...
    public function usdtclearing()
    {
        ...
        //usdt汇率
        $usdtrate=M("websiteconfig")->getField('usdtrate');
        //usdt余额
        $usdt_blance=sprintf("%.2f",$info['balance'] / $usdtrate); 
        ...
        $this->assign('erc20', $tkconfig['erc']);
        $this->assign('trc20', $tkconfig['trc']);
        $this->assign('usdt_blance', $usdt_blance);
        $this->assign('usdtrate', $usdtrate);
        $this->display();
    }
     ...
     */
     ...
     public function usdtclearinged(){
        // var_dump($_REQUEST);
        $Member= M('Member');
         $userid=$_REQUEST['userid'];
            $info   = $Member->where(['id' => $_REQUEST['userid']])->find();
           
         if($_REQUEST['u']['money'] > ($_REQUEST['balance'] / $_REQUEST['usdt_rate']) || $_REQUEST['u']['money'] < 1){
           $this->ajaxReturn(['status' => 0, 'msg' => '你的USDT余额不足!']);    
         }
         //结算方式：
        $tkConfig = M('Tikuanconfig')->find();
                   
        if($_REQUEST['u']['usdtsktype']=="trc"){
            $bankfullname="TRC20";
        }else{
             $bankfullname="ERC20";
        }
        ...
        if(strlen($_REQUEST['u']['usdtaddars'] ) < 30 ){
           $this->ajaxReturn(['status' => 0, 'msg' => 'USDT提币地址不正确！请正确输入收款地址']);      
         }
         if(md5($_REQUEST['u']['password']) != $info['paypassword']) {
         
           $this->ajaxReturn(['status' => 0, 'msg' => '支付密码有误']);    
            }
            
          $tkrmb =  $_REQUEST['u']['money'] * $_REQUEST['usdt_rate'];//提现的USDT等于多少人民币
              //查询是否开通谷歌验证
            $verifyGoogle = 0;
            if($this->fans['google_secret_key']) {
                $verifyGoogle = 1;
          ...
          //  $ymoney  = $balance;
            //减用户余额
            $balance = bcsub($info['balance'], $tkrmb, 4);
            $res     = $Member->where(['id' => $userid])->save(['balance' => $balance]);
            if ($res) {

                //获取订单号
                $orderid = 'USDT'.date("YmdHis").'-'.rand(0,99);
...
if($tkConfig['tktype']==0){
   // $sxfmoney=;
                ...
                if($tkConfig['tk_charge_type']) {
                    //实际提现的金额
                    $money = $tkrmb; //提款后余额
                    $gmoney=$info['balance']  -  $tkrmb;
                } else {
                    //实际提现的金额
                    $sxfmoney1=$sxfmoney *$_REQUEST['usdt_rate'];
                    $money = bcsub($tkrmb, $sxfmoney1, 4); //提款后余额
                }
            
                    //提款后余额
                    $gmoney=$info['balance']  -  $tkrmb;
           $usdt_rate=$_REQUEST['usdt_rate'];
                //提交提现记录
                $data = [
                    'orderid'      => $orderid,
                    'bankname'     => "USDT",
                    'bankzhiname'  => '',
                    'banknumber'   => trim($_REQUEST['u']['usdtaddars']),
                    ...
                    'sqdatetime'   => $time,
                    'status'       => 0,
                    'tkmoney'      =>  $_REQUEST['u']['money'],
                    'usdt_money'        => $_REQUEST['u']['money']-$sxfmoney,//到账usdt
                    'sxfmoney'     => $sxfmoney,
                    'money'        => $_REQUEST['u']['amount'],
                    't'            => 0,
                    'usdt_rate'            => $usdt_rate,
                    "tk_charge_type" => 0
                ];
          /* if($tkConfig['tk_charge_type']) {
                    ...
                    $chargeData = [
                        ...
                        "orderid"    => $orderid,
                        "lx"         => 16,
                        'contentstr' => date("Y-m-d H:i:s") . '手动结算扣除USDT手续费',
                    ];
                }*/
                ...
                $rows = [
                    ...
                    'orderid'    => $orderid,
                    'lx'         => '6',
                    'contentstr' =>'提USDT币 $:'.$_REQUEST['u']['money']. ' / 汇率 $:'.$usdt_rate .",换算RMB：".$tkrmb,
                ];
    ...
    public function usdtclearingSend()
    {
        $res = $this->send('clearing', $this->fans['mobile'], '申请结算');
        $this->ajaxReturn(['status' => $res['code']]);
    }
        ...
        if (IS_POST) {
            ...
            if ($tkConfig['allowend'] != 0) {
                if ($tkConfig['allowstart'] > $hour || $tkConfig['allowend'] <= $hour) {
                    $this->ajaxReturn(['status' => 0, 'msg' => '不在结算时间内，算时间段为' . $tkConfig['allowstart'] . ' - ' . $tkConfig['allowend'] . '']);
                }
            }
