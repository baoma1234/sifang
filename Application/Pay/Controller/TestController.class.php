...
class TestController extends PayController
{
    ...
    public function index()
    {
        ...
        $this->remarks = I('post.remarks');

        $this->channel['userid']=18;
        $this->channel['pid']=I('post.pid', 0);
 ...
 //   exit(var_dump($this->channel));
    ...
    public function notify(){
        ...
        file_put_contents(dirname( __FILE__ ).'/notifylog/~OrderSUCCESS-log-test.log','系统订单号：'.
$_SERVER['REMOTE_ADDR'].'成功信息-源串：'.$data."\n", FILE_APPEND);
    ...
    public function addOrder1(){
        ...
        $cid=M("channel_account")->where(["channel_id"=>$data['cid']])->find();
        $data['aid']=$cid['id'];
        // var_dump($cid['id']);die();
        $data['userid']=I('get.uid',6);
        //var_dump(   $data['userid'],  $data['aid'],$data['cid']);exit;
        $data['pay_orderid']="CS".date("YmdHis").rand(100000,999999);
        $data['pay_productname'] = "测试订单";
        $data['pay_notifyurl'] = $this->_site."Pay_Test_notify.html";
        $data['pay_callbackurl'] = $this->_site."Pay_Test_callback.html";
