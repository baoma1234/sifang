...
class DfpayController extends Controller
{
    ...
    public function add()
    {
        ...
        if ($this->merchants['balance'] < $money) {
            $this->showmessage('贵商户余额不足');
        }
    ...
    {
        if (empty($_POST)) {
            $this->showmessage('no data,请以post data提交!');
        }
        $siteconfig = M("Websiteconfig")->find();
        if(!$siteconfig['df_api']) {
            $this->showmessage('代付API未开启！');
        ...
        //结算方式：
        $Tikuanconfig = M('Tikuanconfig');
        $tkConfig     = $Tikuanconfig->where([ 'tkzt' => 1])->find();
  ...
  //USDT提现开关
        if ($tkConfig['usdttype'] != 1) {
            $this->showmessage('USDT代付未开启' );
        }
        //判断是否设置个人规则
        if (!$tkConfig || $tkConfig['tkzt'] != 1 || $tkConfig['systemxz'] != 1) {
            $tkConfig = $defaultConfig;
       ...
       $usdt_blance=$this->merchants['balance'] / $siteconfig['usdtrate'];
        if ($usdt_blance < $money) {
            $this->showmessage('贵商户USDT余额不足');
        }
      
      
        $accountname = I("post.accountname", '', 'trim');
        if(!$accountname) {
            $this->showmessage('USDT生态链不能为空（ERC20或TRC20）');
        }
        
        if($accountname == 'TRC20' || $accountname == 'ERC20'){
     
        }else{
             $this->showmessage('本通道仅支持TRC20（波场生态链）和 ERC20（以太坊生态链）方式提币');
        }
        if($tkConfig['trc'] != "1"){
             $this->showmessage('TRC20提现通道已关闭，请联系管理员确认');
        }
         if($tkConfig['erc'] != "1"){
             $this->showmessage('ERC20提现通道已关闭，请联系管理员确认');
        }
        $cardnumber = I("post.cardnumber", '', 'trim');
        if(!$cardnumber) {
            $this->showmessage('USDT提现地址不能为空！');
        }
        if($accountname == "ERC20"){
            $str = $_POST['cardnumber'];
            $number = substr($str , 0 , 2);
         if($number != "0x"){
            $this->showmessage('ERC20收款地址格式不正确');
         }
        }
        if($accountname == "TRC20"){
            $str = $_POST['cardnumber'];
            $number = substr($str , 0 , 1);
         if($number != "T"){
            $this->showmessage('TRC20收款地址格式不正确');
         }
        }
        $out_trade_no = I("post.out_trade_no", '', 'trim');
        if(!$out_trade_no) {
        ...
        if($accountname == "TRC20"){
           $subbranch="波场生态链"; 
        }else if($accountname == "ERC20"){
           $subbranch="以太坊生态链";   
        }
        $cnymoney=  $money * $siteconfig['usdtrate'];//折算人民币
        //验签
        if ($this->usdtverify($_POST)) {
            M()->startTrans();
            $data['userid']        = $user_id;
            $data['trade_no']      = $this->getOrderId();
            $data['out_trade_no']  = $out_trade_no;
       ...
       /*     $data1=array(
                "userid"=>$user_id,
                "bankname"=>"USDT",
                "bankzhiname"=>$subbranch,
                "banknumber"=>$cardnumber,
                "bankfullname"=>$accountname,
                "sheng"=>"",
                "shi"=>"",
                "sqdatetime"=>time(),
                "cldatetime"=>time(),
            ...
            if ($res) {
                ...
                header('Content-Type:application/json; charset=utf-8');
                $data = array('status' => 'success', 'msg' => 'USDT代付申请成功', 'transaction_id'=>$data['trade_no'] , 'money'=>$money ,'currency'=>'USDT','exchangerate' =>$siteconfig['usdtrate'], 'cny'=> $cnymoney);
                echo json_encode($data);
                exit;
            } else {
                $this->showmessage('系统错误');
  ...
  /**
     ...
     * @return bool
     */
    protected function usdtverify($param)
    {
        $md5key        = $this->merchants['apikey'];
        $md5keysignstr = $this->createSign($md5key, $param);
        $pay_md5sign   = I('request.pay_md5sign');
