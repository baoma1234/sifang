...
class UserController extends BaseController
{
    ...
    public function index()
    {
      ...
      /* $list = M('ProductUser')
            ...
            ->where(['pay_product_user.userid'=>200282300,'pay_product_user.status'=>1,'pay_product.isdisplay'=>1])
            ->field('pay_product.name,pay_product.id,pay_product_user.status')
            ->select();*/

        foreach ($list as $key=>$item){
            $feilv = M('Userrate')->where(['userid'=>$pid,'payapiid'=>$item['id']])->getField('t0feilv');
            $list[$key]['t0feilv'] = $feilv;
        }
        
    			    
			            $str11=date("Y-m-d")." 00:00:00";
						
						$str222=date("Y-m-d")." 23:59:59";
						foreach ($list as $key=>$item){
						
						
							
							$sql = "select sum(pay_actualamount) as daysum ,pay_bankcode from pay_order where pay_successdate > ".strtotime($str11)." and pay_successdate < ".strtotime($str222)." and pay_memberid = ".($item['id'] +10000)."  and pay_status in (1,2)  group by pay_bankcode";
							$tongdaosum = M('order')->query($sql);

      // var_dump(	$sql);
							$txt = '';
							foreach ($tongdaosum as $vx){
								
								
								$txt .= '<span style="color:ff0000;"><br>通道：'.$vx['pay_bankcode'].'；金额'.$vx['daysum'].'元</span>';
    ...
    public function invitecode()
    {
        ...
        $regdatetime1 = urldecode(I("request.regdatetime"));
		$regdatetime=htmlentities($regdatetime1); //20190413更新过滤特殊字符 反射型xss 中危
        if ($regdatetime) {
        ...
        if (IS_POST) {
            ...
            if ($cztype == 3) {
                ...
                $gmoney          = $info['balance'] + $bgmoney;
                  $res1        = $memberModel->where($where)->save($data);
            } elseif ($cztype == 4) {
                 $data["balance"]  = $info['balance']-$bgmoney;
                // $where['balance'] = array('egt', $bgmoney);
                $gmoney           = $info['balance'] - $bgmoney;
                 $res1 = $memberModel->where($where)->setDec('balance', $bgmoney);
            }
           ...
           // var_dump($res3);
            if ($res1 && $res2 && $res3) {
                M()->commit();
                $this->addAdminLog("商户金额跳动 ID: ".$userid." 金额为:".$bgmoney);
    ...
    public function loginrecord()
    {
        ...
        $logindatetime1 = urldecode(I("request.logindatetime"));
		$logindatetime=htmlentities($logindatetime1); //20190413更新过滤特殊字符 反射型xss 中危
        if ($logindatetime) {
